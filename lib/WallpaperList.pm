...
sub get_global_stats {
	...
	nsfw => $DBH->selectrow_array("select count(*) from wallpaper where nsfw = 1 and deleted is null"),
	sketchy => $DBH->selectrow_array("select count(*) from wallpaper where nsfw = 0 and deleted is null"),
